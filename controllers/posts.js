...
const createPost = async(req, res, next)=>{
    try{
        const {title,coverImg,body} = req.body
...
const updatePost = async(req, res, next)=>{
    try{
        ...
        if(!thepost.author === req.session.user.id) throw Unauthorized('Cannot update this post')
...
const unlikePost = async(req,res,next)=>{
    try{
        ...
        if(!await Posts.findById(req.params.postId)) throw NotFound('No such post found')
        const post = await Posts.findByIdAndUpdate(req.params.postId,{
            $pull:{likes:req.req.session.user.id}
        })
        res.json({success:true})
    }catch(err){
        next(err)
...
const deletePost = async(req,res,next)=>{
    try{
        ...
        if(!post) throw NotFound('No such post found')
        if(!post.author===req.session.user.id) throw Unauthorized('Cannot delete this post')
        //delete the comments
